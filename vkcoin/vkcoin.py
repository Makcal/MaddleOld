...
class VKCoin:
    ...
    def send_payment(self, to_id, amount, mark_as_merchant=True):
        data = {'merchantId': self.user_id, 'key': self.key, 'toId': to_id, 'amount': amount * 1000}
        if mark_as_merchant:
    ...
    def get_payment_url(self, amount, payload=None, free_amount=False):
        ...
        amount = '{:x}'.format(int(amount*1000))
        payload = '{:x}'.format(payload)
        link = f'https://vk.com/app6915965#m{user_id}_{amount}_{payload}'
        if free_amount:
            link += '_1'
        return link
